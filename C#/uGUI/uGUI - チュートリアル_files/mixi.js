...
function add_link(str) {
    ...
    if (!url) {
        ...
        .apply(function(ns){
            ...
            var etag = '</a>';
            if(!ns.isSelected(str)) {
                stag += url;
            }
            $j(str).focus(); // placeholderをclear
            ns.insertText(str, stag, etag);
        });
    }
}
...
Namespace("jp.mixi.common.navigationbar").define(function(ns){
    var menus = [];
    var COMMUNITY_NAVIGATION_BAR = 'personalNaviCommunity';
    var COMMUNITY_NAVIGATION_BAR_DIARY = 'personalNaviCommunity02';
    ...
    var setupCommunityNavigationSubMenu = function(targetName){
        var targetElement = $j('#' + targetName + 'SubMenu');
        $j('#' + targetName).mouseover(function(evt){
            if(evt.target === $j('#' + targetName).find("a").get(0)){
                return;
            }
            targetElement.show();
        });
        $j('#' + targetName + 'PullDownButton').click(function(evt){
            targetElement.show();
            evt.preventDefault();
        });
        $j('#' + targetName).mouseleave(function(evt){
            targetElement.hide();
        });
    };
    ...
    ns.provide({
        registerElement : function(element, dataset){
            ...
            _.each(menus, function(name){
                if(element.className == COMMUNITY_NAVIGATION_BAR ||
                   element.className == COMMUNITY_NAVIGATION_BAR_DIARY){
                ...
                onComplete: function(transport) {
                    ...
                    if ((status >= 200 && status < 300) || status == 1223) {
                        ...
                        Mixi.Popup.SmallConfirm.finish(
                            ...
                            '<p class="supplement01">変更後は<a href="list_appli.pl?mode=manage">表示設定</a>から設定できます。</p>',
...
Mixi.Home.ImageAdjuster.IMG_BASE = 'http://img.mixi.net';
